          ...
          {DIJITAL_DONUSUM_TABS.map((tab) => {
            ...
            return (
              <Link
                ...
                className="card-dark group flex flex-col rounded-2xl p-8 transition-transform duration-200 hover:-translate-y-0.5 hover:border-ege-sea/25"
              >
                <span className="grid size-12 place-items-center rounded-xl bg-ege-sea/15 text-ege-sea-bright ring-1 ring-ege-sea/25 transition-colors group-hover:bg-ege-sea/25">
                  <Icon className="size-6" aria-hidden />
                </span>
                <h3 className="mt-5 text-lg font-semibold leading-snug text-white">
                  {tab.label}
                </h3>
                <p className="mt-3 line-clamp-3 text-left text-[15px] leading-relaxed text-slate-500">
                  {tab.lede}
                </p>
